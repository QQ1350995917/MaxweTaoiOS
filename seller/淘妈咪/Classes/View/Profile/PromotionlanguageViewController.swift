//
//  PromotionlanguageViewController.swift
//  淘妈咪
//
...
import UIKit
import SwiftyButton
class PromotionlanguageViewController: BaseViewController {
    ...
    func saveContent() {
        ...
        if contentText.characters.count == 0 {
            showErrorText(status: "请填写申请佣金理由")
            return
         ...
         */
        let updateReasonUrl = updateReasonAccount()
        ...
        let updateReasonDic = ["t":t,
                           "id":id,
                           "cellphone":cellphone,
                           "apt":1,
                           "sign":sign,
                           "reason":contentText] as [String : Any]
        NetworkTool.shareInstance.request(requestType: .Post, url: updateReasonUrl, parameters: updateReasonDic, succeed: { (result) in
            ...
            if code == 200 {
                self.showErrorText(status: "更新佣金理由成功")
...
extension PromotionlanguageViewController {
    func setupUI() {
        //创建textView
        setupTextView()
        setupSaveBtn()
    ...
    func setupTextView() {
        ...
        NetworkTool.shareInstance.request(requestType: .Post, url: mineUrl, parameters: mineDic, succeed: { (result) in
            ...
            if code == 200 {
                ...
                self.textView.text = MineDto?.reason ?? ""
            }else{
                self.showErrorHud(errorStr: message)
            }
